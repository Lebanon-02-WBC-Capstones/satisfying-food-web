...
function FoodDetails() {
  ...
  const favArrayContains = (food) => {
    const favArray = JSON.parse(localStorage.getItem("fav")) ? JSON.parse(window.localStorage.getItem("fav")) : [];
    return favArray.find(item => item.id === food.id) ? true : false;
  };

  const [favorite, setFavorite] = useState(favArrayContains(currentFood));
  const [heartImage,setHeartImage] = useState(favorite ? "https://image.flaticon.com/icons/png/128/833/833472.png" : "https://image.flaticon.com/icons/png/128/833/833300.png");
  ...
  const handleHeartClick = () => {
    const favArray = JSON.parse(window.localStorage.getItem("fav")) ? JSON.parse(window.localStorage.getItem("fav")) : [];
    if(!favorite) {
    ...
    <div className="">
      ...
      <div>
        ...
        <div className="m-4 ml-2 mb-8 flex flex-wrap">
            {currentFood.category.map(category => <Category
            key = {category}
            image={
              foodData.categoryArray.find(
      ...
      <div>
        ...
        <div className="m-4 ml-2 mb-8 flex flex-wrap">
        {currentFood.ingredients.map(ing => <Category
            key = {ing}
            image={
              portion
            }
